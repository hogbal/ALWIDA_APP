...
const Transportation = ({navigation}) => {
  ...
  useEffect(() => {
    if (
      ...
      getDistance(userLocation.latitude, userLocation.longitude).then(dist => {
  ...
  const getDistance = async (userLatitude, userLongitude) => {
    ...
    var dist =
      Math.sin(raduserLatitude) * Math.sin(radterminalLatitude) +
      Math.cos(raduserLatitude) *
        Math.cos(radterminalLatitude) *
        Math.cos(radTheta);
    if (dist > 1) dist = 1;
        ...
        <View style={styles.info}>
          <View style={styles.header}>
            ...
            <View style={styles.headerText}>
              ...
              <Text style={styles.headerDescriptionText}>
                [{info.terminalAbb}] {info.scale} {info.deviceLocation}
              </Text>
            </View>
          </View>
          ...
          <View style={styles.buttonContainer}>
            <TouchableOpacity
              style={styles.button}
              onPress={() =>
                setReservationModalVisible(!reservationModalVisible)
              }>
            ...
            <CustomModal
              title="예약변경"
              description="예약변경을 하시겠습니까?"
              modalVisible={reservationModalVisible}
              setModalVisible={setReservationModalVisible}
              onpress={resChange}
            />
            <CustomModal
              title="출발취소"
              description="출발취소를 하시겠습니까?"
              modalVisible={departModalVisible}
              setModalVisible={setDepartModalVisible}
              onpress={departCancle}
            />
            <CustomModal
              title="게이트 진입요청"
              description="게이트 진입요청을 하시겠습니까?"
              modalVisible={gateModalVisible}
              setModalVisible={setGateModalVisible}
              onpress={entryRequest}
            />
          </View>
