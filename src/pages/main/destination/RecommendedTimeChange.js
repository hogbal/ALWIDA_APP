...
const RecommendedTime = ({navigation, route}) => {
    ...
    <SafeAreaView style={styles.container}>
      <View style={styles.info}>
        ...
        <View style={styles.timeContainer}>
          <TouchableOpacity
            style={selectedTime == 0 ? styles.selectedTime : styles.time}
            onPress={() => setSelectedTime(0)}>
            <Text style={styles.ampmText}>{data[0].ampm}</Text>
            <Text style={styles.timeText}>
              {data[0].hour}:{data[0].minute}
            </Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={selectedTime == 1 ? styles.selectedTime : styles.time}
            onPress={() => setSelectedTime(1)}>
            <Text style={styles.ampmText}>{data[1].ampm}</Text>
            <Text style={styles.timeText}>
              {data[1].hour}:{data[1].minute}
            </Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={selectedTime == 2 ? styles.selectedTime : styles.time}
            onPress={() => setSelectedTime(2)}>
            <Text style={styles.ampmText}>{data[2].ampm}</Text>
            <Text style={styles.timeText}>
              {data[2].hour}:{data[2].minute}
            </Text>
          </TouchableOpacity>
        </View>
      </View>
